...
def populate_month_choices(form, bool_view_all=False):
    subq = (
        db.select(
...
def brs_tieup_dashboard():
    ...
    if user_type in {"admin", "ro_user"}:
        ...
        populate_month_choices(form, bool_view_all=True)
...
def upload_document_to_folder(
    *, brs_tieup_obj, model_attribute, form, field, folder_name
):
...
def brs_tieup_data_entry(key):
    ...
    if form.validate_on_submit():
        ...
        # )
        upload_document_to_folder(
            brs_tieup_obj=brs_entry,
            model_attribute="bank_statement",
            form=form,
            field="file_bank_statement",
            folder_name="brs_tieup_bank_statement",
        )
        db.session.commit()
...
def brs_auto_upload_prev_month():
    ...
    stmt = db.select(
        ...
        BankReconTieupSummary.gl_identifier,
        BankReconTieupSummary.tieup_partner_name,
        BankReconTieupSummary.tieup_bank_name,
        BankReconTieupSummary.tieup_bank_account_number,
        BankReconTieupSummary.tieup_gc_gl_code,
        BankReconTieupSummary.tieup_bank_ifsc_code,
        BankReconTieupSummary.tieup_gc_bank_code,
        BankReconTieupSummary.tieup_gc_branch_code,
        BankReconTieupSummary.tieup_gc_bank_name,
        BankReconTieupSummary.tieup_bank_branch_name,
        BankReconTieupSummary.tieup_bank_branch_address,
    ...
    insert_stmt = db.insert(BankReconTieupSummary).from_select(
        [
            ...
            BankReconTieupSummary.tieup_bank_branch_name,
            BankReconTieupSummary.tieup_bank_branch_address,
            BankReconTieupSummary.created_by,
        ],
        stmt,
    )
...
def bulk_upload_brs():
    ...
    if form.validate_on_submit():
        ...
        db.session.execute(
            db.insert(BankReconTieupSummary), df.to_dict(orient="records")
        )
        db.session.commit()
        # df.to_sql(
        ...
        #     index=False,
        # )
        flash("BRS tieup records have been uploaded to database")

    return render_template(
        "brs_tieup_form.html", form=form, title="Bulk upload tieup items"
    )
...
def list_brs_entries():
    ...
    return render_template(
        "brs_tieup_form.html", form=form, title="Fetch BRS - tieup data"
    )

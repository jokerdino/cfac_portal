...
class BankStatementServiceCBX:
    ...
    def process(self, file, user):
        df, closing_balance_stmt = self.parse_bank_statement(file)
        df = self.normalize_bank_statement(df, user)
        df_flag_sheet = self.get_flag_sheet()
        df = self.add_flag(df, df_flag_sheet)

        self.verify_closing_balance(df, closing_balance_stmt)
        self.save_bank_statement_and_credits(df, closing_balance_stmt)
    ...
    def parse_bank_statement(self, file) -> pd.DataFrame:
        ...
        closing_balance = df_summary.loc[mask, 1].iloc[0]
        df = pd.read_excel(
            file,
            skiprows=14,
    ...
    def verify_closing_balance(self, df, closing_balance_stmt) -> bool:
    ...
    def save_bank_statement_and_credits(self, df: pd.DataFrame, closing_balance_stmt):
        try:
        ...
        # ].item()
        self.create_or_update_daily_sheet(closing_balance_stmt)
        self.session.commit()
